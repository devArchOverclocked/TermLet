...
local function get_help_lines(width)
  ...
  return {
    ...
    "    " .. string.rep("─", sep_width),
    "",
    "    Press keys in sequence (real-time capture)",
...
local function format_keybinding_line(script, keybinding, index, is_selected, width)
  ...
  local name_width = math.max(math.floor(width * 0.30), 12)
  local key_width = math.max(math.floor(width * 0.25), 12)
  ...
  local line = pointer .. number_label .. padded_name .. "  " .. padded_key

  local regions = {}

  -- Track key display region for highlighting
  local key_start = #pointer + #number_label + #padded_name + 2
  local key_end = key_start + #key_display
  if keybinding and keybinding ~= "" then
    table.insert(regions, { col_start = key_start, col_end = key_end, group = "keybinding" })
  else
    table.insert(regions, { col_start = key_start, col_end = key_end, group = "notset" })
  end
...
local function render_ui()
  ...
  if state.mode == "capture" then
    ...
    table.insert(highlights, { line = #lines - 1, group = state.config.highlight.title })
    ...
    table.insert(lines, "    Escape to cancel, Enter to confirm")
    table.insert(highlights, { line = #lines - 1, group = state.config.highlight.help })
    table.insert(lines, "")
    table.insert(lines, "    " .. string.rep("─", sep_width))
    table.insert(lines, "")
  ...
  elseif state.show_help then
    ...
    if #state.scripts == 0 then
      ...
      for i, script in ipairs(state.scripts) do
        ...
        local line, regions = format_keybinding_line(script, keybinding, i, is_selected, width)
        ...
        -- Apply per-region highlights when not selected

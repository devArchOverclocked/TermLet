-- Tests for TermLet filter UI module
-- Run with: nvim --headless -c "PlenaryBustedDirectory tests/ {minimal_init = 'tests/minimal_init.lua'}"

local filter_ui = require("termlet.filter_ui")

describe("termlet.filter_ui", function()
  local test_buf

  before_each(function()
    -- Close any existing UI
    filter_ui.close()
    -- Create a valid target buffer
    test_buf = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_buf_set_lines(test_buf, 0, -1, false, { "error: test", "info: test" })
  end)

  after_each(function()
    filter_ui.close()
    if test_buf and vim.api.nvim_buf_is_valid(test_buf) then
      vim.api.nvim_buf_delete(test_buf, { force = true })
    end
  end)

  describe("open", function()
    it("should open the filter UI", function()
      local result = filter_ui.open(test_buf)
      assert.is_true(result)
      assert.is_true(filter_ui.is_open())
    end)

    it("should return false for invalid buffer", function()
      local result = filter_ui.open(9999)
      assert.is_false(result)
    end)

    it("should return false for nil buffer", function()
      local result = filter_ui.open(nil)
      assert.is_false(result)
    end)

    it("should close and return false when already open", function()
      filter_ui.open(test_buf)
      assert.is_true(filter_ui.is_open())

      local result = filter_ui.open(test_buf)
      assert.is_false(result)
    end)

    it("should use proper floating window border", function()
      filter_ui.open(test_buf)
      assert.is_true(filter_ui.is_open())
    end)

    it("should initialize state correctly", function()
      filter_ui.open(test_buf)
      local state = filter_ui.get_state()

      assert.equals(1, state.selected_index)
      assert.equals("all", state.current_preset)
      assert.equals(4, state.preset_count)
    end)

    it("should initialize selected_index from current_preset", function()
      -- Open and navigate to preset 3, then apply
      filter_ui.open(test_buf)
      filter_ui.actions.move_down()
      filter_ui.actions.move_down()
      local state = filter_ui.get_state()
      assert.equals(3, state.selected_index)

      -- Close UI (selected_index resets, but current_preset persists if applied)
      filter_ui.close()

      -- Reopen - selected_index should be 1 since we reset on close
      filter_ui.open(test_buf)
      state = filter_ui.get_state()
      assert.equals(1, state.selected_index)
    end)
  end)

  describe("close", function()
    it("should close the filter UI", function()
      filter_ui.open(test_buf)
      assert.is_true(filter_ui.is_open())

      filter_ui.close()
      assert.is_false(filter_ui.is_open())
    end)

    it("should not error when no UI is open", function()
      filter_ui.close()
      assert.is_false(filter_ui.is_open())
    end)

    it("should reset selected_index on close", function()
      filter_ui.open(test_buf)
      filter_ui.actions.move_down()
      filter_ui.actions.move_down()

      local state = filter_ui.get_state()
      assert.equals(3, state.selected_index)

      filter_ui.close()

      -- After close, selected_index should be reset to 1
      state = filter_ui.get_state()
      assert.equals(1, state.selected_index)
    end)
  end)

  describe("is_open", function()
    it("should return false initially", function()
      assert.is_false(filter_ui.is_open())
    end)

    it("should return true when open", function()
      filter_ui.open(test_buf)
      assert.is_true(filter_ui.is_open())
    end)

    it("should return false after closing", function()
      filter_ui.open(test_buf)
      filter_ui.close()
      assert.is_false(filter_ui.is_open())
    end)
  end)

  describe("toggle", function()
    it("should open when closed", function()
      filter_ui.toggle(test_buf)
      assert.is_true(filter_ui.is_open())
    end)

    it("should close when open", function()
      filter_ui.open(test_buf)
      assert.is_true(filter_ui.is_open())

      filter_ui.toggle(test_buf)
      assert.is_false(filter_ui.is_open())
    end)
  end)

  describe("navigation", function()
    before_each(function()
      filter_ui.open(test_buf)
    end)

    it("should start at index 1", function()
      local state = filter_ui.get_state()
      assert.equals(1, state.selected_index)
    end)

    it("should move down correctly", function()
      filter_ui.actions.move_down()
      local state = filter_ui.get_state()
      assert.equals(2, state.selected_index)

      filter_ui.actions.move_down()
      state = filter_ui.get_state()
      assert.equals(3, state.selected_index)
    end)

    it("should wrap around when moving down past last item", function()
      -- Move to last preset (4th)
      filter_ui.actions.move_down()
      filter_ui.actions.move_down()
      filter_ui.actions.move_down()
      local state = filter_ui.get_state()
      assert.equals(4, state.selected_index)

      -- Move down should wrap to first
      filter_ui.actions.move_down()
      state = filter_ui.get_state()
      assert.equals(1, state.selected_index)
    end)

    it("should move up correctly", function()
      filter_ui.actions.move_down()
      filter_ui.actions.move_down()
      local state = filter_ui.get_state()
      assert.equals(3, state.selected_index)

      filter_ui.actions.move_up()
      state = filter_ui.get_state()
      assert.equals(2, state.selected_index)
    end)

    it("should wrap around when moving up past first item", function()
      local state = filter_ui.get_state()
      assert.equals(1, state.selected_index)

      -- Move up should wrap to last
      filter_ui.actions.move_up()
      state = filter_ui.get_state()
      assert.equals(4, state.selected_index)
    end)
  end)

  describe("get_state", function()
    it("should return current state", function()
      filter_ui.open(test_buf)
      local state = filter_ui.get_state()

      assert.is_number(state.selected_index)
      assert.is_string(state.current_preset)
      assert.is_number(state.preset_count)
    end)

    it("should reflect navigation changes", function()
      filter_ui.open(test_buf)
      filter_ui.actions.move_down()

      local state = filter_ui.get_state()
      assert.equals(2, state.selected_index)
    end)

    it("should show default preset as all", function()
      local state = filter_ui.get_state()
      assert.equals("all", state.current_preset)
    end)
  end)
end)

...
describe("termlet.watch", function()
  ...
  describe("is_excluded", function()
    ...
    it("should work with pre-built exclude set", function()
      local exclude_set = watch._build_exclude_set({ "node_modules", ".git", "build" })
      assert.is_true(watch._is_excluded("node_modules", exclude_set))
      assert.is_true(watch._is_excluded("path/node_modules/file", exclude_set))
      assert.is_true(watch._is_excluded("build", exclude_set))
      assert.is_false(watch._is_excluded("rebuild", exclude_set))
      assert.is_false(watch._is_excluded("src", exclude_set))
    end)
  end)

  describe("build_exclude_set", function()
    it("should build a set from a list", function()
      local set = watch._build_exclude_set({ "node_modules", ".git" })
      assert.is_true(set["node_modules"])
      assert.is_true(set[".git"])
      assert.is_nil(set["src"])
    end)

    it("should return empty set for nil input", function()
      local set = watch._build_exclude_set(nil)
      assert.is_nil(next(set))
    end)

    it("should return empty set for empty list", function()
      local set = watch._build_exclude_set({})
      assert.is_nil(next(set))
    end)
  end)
